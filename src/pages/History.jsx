...
//       try {
//         const response = await axios.get("http://localhost:8000/file-history");
//         const data = response.data.map((item, index) => ({
//           id: index + 1,
//           filename: item.filename,
//           source: item.bucket,
//           subSource: item.content_type,
//           uploadDate: item.timestamp,
//           size: (item.size / (1024 * 1024)).toFixed(2) + " MB",
//           status: "Completed",  // Set based on logic if needed
//         }));
    ...
    const fetchHistory = async () => {
      try {
        ...
        const formatted = data.map((item, index) => ({
          ...
          source: item.source,  // Now a name like "PIRACY"
